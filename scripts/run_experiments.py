...
def completed_example_ids(path: Path) -> set[str]:
    """Return valid completed example IDs from a possibly interrupted JSONL run."""
    if not path.exists():
        return set()
    completed = set()
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise ValueError(
                    f"Cannot resume {path}: malformed JSON on line {line_number}"
                ) from exc
            key = str(row.get("example_id") or row.get("study_id") or "")
            if not key:
                raise ValueError(f"Cannot resume {path}: line {line_number} has no example ID")
            if key in completed:
                raise ValueError(f"Cannot resume {path}: duplicate example ID {key}")
            completed.add(key)
    return completed
...
def main() -> None:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="append only missing examples to existing JSONL outputs",
    )
    ...
    remaining_test = [
        study for study in selected_test if manifest_example_id(study) not in completed
    ]
    if completed:
...
def _run_suite(args, test, index, linker, graph, configs, drafts, index_load_ms):
    ...
    selected_test = test[: args.limit]
    completed_by_ablation = {
        name: completed_example_ids(args.output / f"{name}.jsonl") if args.resume else set()
        for name, _, _, _ in entries
    }
    completed_runs = sum(len(values) for values in completed_by_ablation.values())
    if completed_runs:
        print(
            f"Resuming suite: {completed_runs} ablation/example runs already complete",
            flush=True,
        )
    with ExitStack() as stack:
        handles = {
            name: stack.enter_context(
                (args.output / f"{name}.jsonl").open("a" if args.resume else "w", encoding="utf-8")
            )
            for name, _, _, _ in entries
        }
        study_progress = tqdm(
        ...
        ablation_progress = tqdm(
            total=len(selected_test) * len(entries) - completed_runs,
        ...
        for study in study_progress:
            ...
            for name, config, controlled_graph, control in entries:
                if replay_key in completed_by_ablation[name]:
                    continue
