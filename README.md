...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/FaezehMillerAI/Adopted-AAAI/blob/main/notebooks/Adaptive_NeSy_Gen_Experiments_Colab.ipynb)
...
### Fastest defensible A100 run order

1. Run Cells 1–4 with `RUN_DATASET="iuxray_official"`; existing valid Drive caches are
   reused automatically.
2. Run a 20-step QLoRA and a small `LIMIT` first as an end-to-end systems check. Do not
   report those smoke metrics.
3. Resume QLoRA at 500 steps, set `LIMIT=None`, and generate the official test split once.
4. Replay the saved draft through the complete verifier suite; this avoids nine extra
   generator passes.
5. Run publication evaluation, then repeat the dataset-specific cells for `mimic_aug`.
...
Long generation and replay commands in the Colab notebook use `--resume`. Completed
JSONL rows are retained after a runtime disconnect, validated for duplicate/malformed
records, and skipped on rerun. This is especially important for a one-day A100 session:
build each cache once, generate each test image once, and replay the saved drafts for all
verifier ablations without loading the generator again.
